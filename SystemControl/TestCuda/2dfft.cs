...
namespace Fast_Fourier_Transform
{
    ...
    class FFT
    {
        ...
        */
        ...
        public static void DoFFT(double[] reals, double[] imag)
        {
            uint logn = (uint)Math.Ceiling(Math.Log(reals.Length * 1.0, 2));
            int vectorLength = (int)Math.Pow(2, logn);
            double[] ra = new double[(int)vectorLength];
            double[] im = new double[(int)vectorLength];
            ...
            FFT1D(1, (int)logn, ref ra, ref im);
            for (int i = 0; i < reals.Length; i++)
            {
                reals[i] = ra[i];
                imag[i] = im[i];
